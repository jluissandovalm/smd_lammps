...
PairULSPHHeat::PairULSPHHeat(LAMMPS *lmp) :
		Pair(lmp) {
...
void PairULSPHHeat::PreCompute_DensitySummation() {
...
void PairULSPHHeat::compute(int eflag, int vflag) {
	double **x = atom->x;
	double **v = atom->v;
	double **f = atom->f;
	double *vfrac = atom->vfrac;
...
// zero accumulators
	...
	if (velocity_gradient) {
		PairULSPHHeat::PreCompute(); // get velocity gradient and kernel gradient correction
	}

	PairULSPHHeat::AssembleStressTensor();
...
/* ----------------------------------------------------------------------
 ...
 ------------------------------------------------------------------------- */
void PairULSPHHeat::AssembleStressTensor() {
	double *radius = atom->radius;
	double *vfrac = atom->vfrac;
	double *rmass = atom->rmass;
...
void PairULSPHHeat::allocate() {
	...
	memory->create(DIFFUSIV,  n + 1, "pair:diffusiv");
...
void PairULSPHHeat::settings(int narg, char **arg) {
	if (narg != 4) {
		printf("narg = %d\n", narg);
	...
	if (strcmp(arg[3], "*HEAT") == 0) {
			heat = true;
			if (comm->me == 0)
				printf("... Heat Transfer (Thermal Diffusion) is active\n");
	} else if (strcmp(arg[3], "*NO_HEAT") == 0) {
			heat = false;
			if (comm->me == 0)
				printf("... Heat Transfer (Thermal Diffusion)  is NOT active\n");
	} else {
			error->all(FLERR, "Illegal settings keyword for pair style ulsph");
	}
...
void PairULSPHHeat::coeff(int narg, char **arg) {
	...
	/*
	 * if parameters are given in i,i form, i.e., no a cross interaction, set material parameters
	 */
	...
	if (force->inumeric(FLERR, arg[0]) == force->inumeric(FLERR, arg[1])) {
		...
		if (iNextKwd - ioffset != 6 + 1) {
			sprintf(str, "expected 6 arguments following *COMMON but got %d\n", iNextKwd - ioffset - 1);
			error->all(FLERR, str);
		}
		...
		DIFFUSIV[itype]                            = force->numeric(FLERR, arg[ioffset + 5]);
		Lookup[HOURGLASS_CONTROL_AMPLITUDE][itype] = force->numeric(FLERR, arg[ioffset + 6]);

		Lookup[BULK_MODULUS][itype] = Lookup[REFERENCE_SOUNDSPEED][itype]
		...
		if (comm->me == 0) {
			...
			printf(FORMAT1,                           "thermal diffusivity", DIFFUSIV[itype]);
...
double PairULSPHHeat::init_one(int i, int j) {
...
void PairULSPHHeat::init_list(int id, NeighList *ptr) {
	if (id == 0)
		list = ptr;
}
...
double PairULSPHHeat::memory_usage() {
...
int PairULSPHHeat::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc) {
	double *vfrac = atom->vfrac;
	int i, j, m;
...
void PairULSPHHeat::unpack_forward_comm(int n, int first, double *buf) {
	double *vfrac = atom->vfrac;
	int i, m, last;
...
void *PairULSPHHeat::extract(const char *str, int &i) {
...
double PairULSPHHeat::effective_shear_modulus(const Matrix3d d_dev, const Matrix3d deltaStressDev, const double dt, const int itype) {
