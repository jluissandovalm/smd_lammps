...
ComputeStressAtom::ComputeStressAtom(LAMMPS *lmp, int narg, char **arg) :
	...
	if (strcmp(arg[3], "NULL") == 0)
		...
		if (modify->compute[icompute]->tempflag == 0)
			error->all(FLERR, "Compute stress/atom temperature ID does not "
					"compute temperature");
	}
...
void ComputeStressAtom::compute_peratom() {
	...
	// if this is a continuum mechanics simulation, the volume is know.
	// divide with volume such that the stress tensor has units of pressure
	if (atom->smd_flag) {
